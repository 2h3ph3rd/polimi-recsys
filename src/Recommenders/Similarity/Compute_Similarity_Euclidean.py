    ...
    def compute_similarity(self, start_col=None, end_col=None, block_size=100):
        ...
        while start_col_block < end_col_local:
            ...
            for col_index_in_block in range(this_block_size):
                ...
                if self.use_row_weights:
                    item_distance = np.multiply(
                        item_distance, self.row_weights)

                if self.normalize:
                    denominator = sumOfSquared[columnIndex] * sumOfSquared
                    item_distance[denominator !=
                                  0.0] /= denominator[denominator != 0.0]
                ...
                if self.similarity_is_exp:
                    item_similarity = 1 / \
                        (np.exp(item_distance) + self.shrink + 1e-9)
                    ...
                    item_similarity = 1 / \
                        (np.log(item_distance+1) + self.shrink + 1e-9)
