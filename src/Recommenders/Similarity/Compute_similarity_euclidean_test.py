...
class MyTestCase(unittest.TestCase):
    ...
    def test_euclidean_similarity_integer(self):
        ...
        similarity_object = Compute_Similarity_Euclidean(sps.csr_matrix(
            data_matrix).T, topK=100, normalize=False, similarity_from_distance_mode="lin")
        ...
        for vector1 in range(n_items):
            for vector2 in range(n_items):
                ...
                scipy_distance = euclidean(
                    data_matrix[vector1, :], data_matrix[vector2, :])

                if vector1 == vector2:
                    assert W_local[vector1, vector2] == 0.0, "W_local[{},{}] not matching control".format(
                        vector1, vector2)

                else:
                    local_similarity = 1/W_local[vector1, vector2]

                    assert np.allclose(local_similarity, scipy_distance,
