...
from src.Data_manager.DataReader_utils import reconcile_mapper_with_removed_tokens, remove_features
from src.Data_manager.data_consistency_check import assert_URM_ICM_mapper_consistency
from src.Data_manager.DataReader_utils import compute_density
...
class Dataset(object):
    ...
    def _assert_is_initialized(self):
        assert self.AVAILABLE_URM is not None, "DataReader {}: Unable to load data split. The split has not been generated yet, call the load_data function to do so.".format(
    ...
    def verify_data_consistency(self):
        ...
        for URM_name, URM_object in self.AVAILABLE_URM.items():
            assert URM_object.nnz != 0, print_preamble + \
                "Number of interactions in {} is 0".format(URM_name)

        if self.is_implicit():
            assert np.all(URM_all.data == 1.0), print_preamble + \
                "The DataReader is stated to be implicit but the main URM is not"
    ...
    def _remove_items_and_users(self, items_to_remove=None, users_to_remove=None):
        ...
        if items_to_remove is not None:
            ...
            self.item_original_ID_to_index = reconcile_mapper_with_removed_tokens(
                self.item_original_ID_to_index, items_to_remove)
        ...
        if users_to_remove is not None:
            ...
            self.user_original_ID_to_index = reconcile_mapper_with_removed_tokens(
                self.user_original_ID_to_index, users_to_remove)
