...
import torch
from torch.src.Utils.data import Dataset, DataLoader
...
class MF_MSE_PyTorch_model(torch.nn.Module):
    ...
    def __init__(self, n_users, n_items, n_factors):
        ...
        self.USER_factors = torch.nn.Embedding(
            num_embeddings=self.n_users, embedding_dim=self.n_factors)
        self.ITEM_factors = torch.nn.Embedding(
            num_embeddings=self.n_items, embedding_dim=self.n_factors)

        self.layer_1 = torch.nn.Linear(
            in_features=self.n_factors, out_features=1)
...
class DatasetIterator_URM(Dataset):
    ...
    def __init__(self, URM):
        ...
        self.user_item_coordinates = torch.Tensor(
            self.user_item_coordinates).type(torch.LongTensor)
