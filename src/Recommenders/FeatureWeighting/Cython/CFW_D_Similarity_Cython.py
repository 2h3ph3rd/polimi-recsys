...
class EvaluatorCFW_D_wrapper(object):
    ...
    def __init__(self, evaluator_object, ICM_target, model_to_use="best"):
        ...
        assert model_to_use in [
            "best", "last"], "EvaluatorCFW_D_wrapper: model_to_use must be either 'best' or 'incremental'. Provided value is: '{}'".format(model_to_use)

        self.model_to_use = model_to_use

    def evaluateRecommender(self, recommender_object):

        recommender_object.set_ICM_and_recompute_W(
            self.ICM_target, recompute_w=False)
...
class CFW_D_Similarity_Cython(BaseItemCBFRecommender, BaseItemSimilarityMatrixRecommender, Incremental_Training_Early_Stopping):
    ...
    def fit(self, show_max_performance=False,
        ...
        from src.Recommenders.FeatureWeighting.Cython.CFW_D_Similarity_Cython_SGD import CFW_D_Similarity_Cython_SGD
    ...
    def _generate_train_data(self):
        ...
        self.similarity = Compute_Similarity(
            self.ICM.T, shrink=0, topK=self.topK, normalize=False)
        ...
        estimated_n_samples = int(
            S_matrix_contentKNN.nnz*(1+self.add_zeros_quota)*1.2)
            ...
            for index in range(len(is_common)):
                ...
                if num_samples == estimated_n_samples:
                    ...
                    self.data_list = np.concatenate(
                        (self.data_list, np.zeros(dataBlock, dtype=np.float64)))
