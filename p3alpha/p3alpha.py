...
from p3alpha import check_matrix, similarityMatrixTopK
from p3alpha import seconds_to_biggest_unit

from p3alpha import BaseItemSimilarityMatrixRecommender
...
class P3alphaRecommender(BaseItemSimilarityMatrixRecommender):
    ...
    def fit(self, topK=100, alpha=1., min_rating=0, implicit=False, normalize_similarity=False):
        ...
        if self.min_rating > 0:
            ...
            if self.implicit:
                self.URM_train.data = np.ones(
                    self.URM_train.data.size, dtype=np.float32)
        ...
        similarity_builder = Incremental_Similarity_Builder(
            Pui.shape[1], initial_data_block=Pui.shape[1]*self.topK, dtype=np.float32)
        ...
        for current_block_start_row in range(0, Pui.shape[1], block_dim):
            ...
            similarity_block = d_t[current_block_start_row:
                                   current_block_start_row + block_dim, :] * Pui
            ...
            for row_in_block in range(block_dim):
                ...
                relevant_items_partition = np.argpartition(
                    -row_data, self.topK-1, axis=0)[:self.topK]
            ...
            if time.time() - start_time_printBatch > 300 or current_block_start_row + block_dim == Pui.shape[1]:
                ...
                self._print("Similarity column {} ({:4.1f}%), {:.2f} column/sec. Elapsed time {:.2f} {}".format(
                    ...
                    100.0 * float(current_block_start_row +
                                  block_dim) / Pui.shape[1],
