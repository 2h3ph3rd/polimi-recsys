...
from src.Recommenders.BaseCBFRecommender import BaseItemCBFRecommender
...
from src.Recommenders.IR_feature_weighting import okapi_BM_25, TF_IDF
from src.Recommenders.Similarity.Compute_Similarity import Compute_Similarity
...
class CFW_DVV_Similarity_Cython(BaseItemCBFRecommender, BaseItemSimilarityMatrixRecommender, Incremental_Training_Early_Stopping):
    ...
    def fit(self,
        ...
        if initialization_mode_D not in self.INIT_TYPE_VALUES:
            raise ValueError("Value for 'initialization_mode_D' not recognized. Acceptable values are {}, provided was '{}'".format(
                self.INIT_TYPE_VALUES, initialization_mode_D))
        ...
        if initialization_mode_D == "random":
            ...
            weights_initialization_D = np.ones(
                self.n_features, dtype=np.float64)
        elif initialization_mode_D == "zero":
            weights_initialization_D = np.zeros(
                self.n_features, dtype=np.float64)
    ...
    def _generate_train_data(self):
        ...
        if zeros_to_add != 0.0:
            self.row_list = np.concatenate((np.array(
                self.S_matrix_target.row, dtype=np.int32), np.zeros(zeros_to_add, dtype=np.int32)))
            self.col_list = np.concatenate((np.array(
                self.S_matrix_target.col, dtype=np.int32), np.zeros(zeros_to_add, dtype=np.int32)))
            self.data_list = np.concatenate((np.array(
                self.S_matrix_target.data, dtype=np.float64), np.zeros(zeros_to_add, dtype=np.float64)))
            ...
            self.data_list = np.array(
                self.S_matrix_target.data, dtype=np.float64)
    ...
    def _add_zeros_in_train_data_row_wise(self):
        ...
        if zeros_to_add_global + numSamples >= n_items**2:
            raise ValueError(
                self.RECOMMENDER_NAME + ": Too many zeros to add, not enough unique coordinates in matrix")
        ...
        if self.verbose:
            ...
            print(self.RECOMMENDER_NAME +
                  ": Added zeros, data points are {}".format(len(self.data_list)))

    def compute_W_sparse(self, use_D=True, use_V=True,  model_to_use="best"):

        assert model_to_use in [
            "last", "best"], "{}: compute_W_sparse, 'model_to_use' parameter not recognized".format(self.RECOMMENDER_NAME)
        ...
        if use_D:
            ...
            similarity = Compute_Similarity(
                self.ICM.T, shrink=0, topK=self.topK, normalize=False, row_weights=D)
        ...
        if use_V:
            ...
            for numItem in range(self.n_items):
                ...
                relevant_items_partition = (-V_weights).argpartition(
    ...
    def save_model(self, folder_path, file_name=None):
        ...
        print("{}: Saving model in file '{}'".format(
