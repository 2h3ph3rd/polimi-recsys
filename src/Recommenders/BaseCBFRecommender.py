...
from src.Recommenders.BaseRecommender import BaseRecommender as _BaseRecommender
...
class BaseItemCBFRecommender(_BaseRecommender):
    ...
    def __init__(self, URM_train, ICM_train, verbose=True):
        super(BaseItemCBFRecommender, self).__init__(
        ...
        self.ICM_train = check_matrix(
            ICM_train.copy(), 'csr', dtype=np.float32)
...
class BaseUserCBFRecommender(_BaseRecommender):
    ...
    def __init__(self, URM_train, UCM_train, verbose=True):
        super(BaseUserCBFRecommender, self).__init__(
            URM_train, verbose=verbose)

        assert self.n_users == UCM_train.shape[0], "{}: URM_train has {} users but UCM_train has {}".format(
            self.RECOMMENDER_NAME, self.n_items, UCM_train.shape[0])

        self.UCM_train = check_matrix(
            UCM_train.copy(), 'csr', dtype=np.float32)
