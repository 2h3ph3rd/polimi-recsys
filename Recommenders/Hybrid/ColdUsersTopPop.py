...
from ..NonPersonalizedRecommender import TopPop
...
class ColdUsersTopPop(BaseRecommender):
    ...
    def __init__(
        ...
        self.RECOMMENDER_NAME = warm_recommender.RECOMMENDER_NAME[:-
                                                                  11]+'ColdUsersTopPopRecommender'

        self.warm_recommender = warm_recommender

        self.cold_recommender = TopPop(URM_train=URM_train)

        super(ColdUsersTopPop, self).__init__(
            URM_train=URM_train, verbose=verbose)
