...
class TopPop(BaseRecommender):
    ...
    def _compute_item_score(self, user_id_array, items_to_compute=None):
        ...
        item_scores = np.array(
            item_pop_to_copy, dtype=np.float32).reshape((1, -1))
...
class GlobalEffects(BaseRecommender):
    ...
    def fit(self, lambda_user=10, lambda_item=25):
        ...
        self.mu = self.URM_train.data.sum(
            dtype=np.float32) / self.URM_train.nnz
        ...
        self.item_bias = URM_train_unbiased.sum(
            axis=0) / (col_nnz + self.lambda_item)
    ...
    def _compute_item_score(self, user_id_array, items_to_compute=None):
        ...
        if items_to_compute is not None:
            item_bias_to_copy = - \
                np.ones(self.n_items, dtype=np.float32)*np.inf
        ...
        item_scores = np.array(
            item_bias_to_copy, dtype=np.float).reshape((1, -1))
        item_scores = np.repeat(item_scores, len(user_id_array), axis=0)
...
class Random(BaseRecommender):
    ...
    def _compute_item_score(self, user_id_array, items_to_compute=None):
        ...
        if items_to_compute is not None:
            ...
            item_scores[:, items_to_compute] = np.random.rand(
                len(user_id_array), len(items_to_compute))
