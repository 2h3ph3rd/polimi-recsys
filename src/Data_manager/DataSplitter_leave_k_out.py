...
from src.Data_manager.split_functions.split_train_validation_leave_k_out import split_train_leave_k_out_user_wise
...
class DataSplitter_leave_k_out(_DataSplitter):
    ...
    def __init__(self, dataReader_object: _DataReader, k_out_value=1, forbid_new_split=False, force_new_split=False, use_validation_set=True, leave_random_out=True):
        ...
        assert k_out_value >= 1, "{}: k_out_value must be  greater or equal than 1".format(
        ...
        super(DataSplitter_leave_k_out, self).__init__(dataReader_object,
    ...
    def _save_split(self, save_folder_path):
        ...
        if save_folder_path:
            ...
            name_suffix = "_{}_{}".format(
                allow_cold_users_suffix, validation_set_suffix)
    ...
    def _verify_data_consistency(self):
        ...
        assert all(URM_name in self.SPLIT_URM_DICT for URM_name in URM_to_load_list), print_preamble + \
            "Not all URMs have been created"
        assert all(URM_name in URM_to_load_list for URM_name in self.SPLIT_URM_DICT.keys(
        ...
        if self.use_validation_set:
            assert self.SPLIT_URM_DICT["URM_validation"].nnz != 0, print_preamble + \
                "Number of interactions in URM Validation is 0"
