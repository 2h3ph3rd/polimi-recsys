...
import p3alpha as recommender_utils
from p3alpha import seconds_to_biggest_unit
...
class Incremental_Similarity_Builder:
    ...
    def __init__(self, matrix_size, initial_data_block=10000000, additional_data_block=10000000, dtype=np.float32):
        ...
        self._max_value_of_coordinate_dtype = np.iinfo(
            self._dtype_coordinates).max

        self._row_array = np.zeros(
            self._initial_data_block, dtype=self._dtype_coordinates)
        self._col_array = np.zeros(
            self._initial_data_block, dtype=self._dtype_coordinates)
        self._data_array = np.zeros(
            self._initial_data_block, dtype=self._dtype_data)
    ...
    def add_data_lists(self, row_list_to_add, col_list_to_add, data_list_to_add):
        ...
        for data_point_index in range(len(row_list_to_add)):
            ...
            if self._next_cell_pointer == len(self._row_array):
                self._row_array = np.concatenate((self._row_array, np.zeros(
                    self._additional_data_block, dtype=self._dtype_coordinates)))
                self._col_array = np.concatenate((self._col_array, np.zeros(
                    self._additional_data_block, dtype=self._dtype_coordinates)))
                self._data_array = np.concatenate((self._data_array, np.zeros(
                    self._additional_data_block, dtype=self._dtype_data)))
...
class Compute_Similarity_Python:
    ...
    def __init__(self, dataMatrix, topK=100, shrink=0, normalize=True,
        ...
        if row_weights is not None:
            ...
            self.dataMatrix_weighted = self.dataMatrix.T.dot(
                self.row_weights_diag).T
    ...
    def compute_similarity(self, start_col=None, end_col=None, block_size=100):
        ...
        similarity_builder = Incremental_Similarity_Builder(
            self.n_columns, initial_data_block=self.n_columns*self.topK, dtype=np.float32)
        ...
        if self.asymmetric_cosine:
            sum_of_squared_to_alpha = np.power(
                sum_of_squared + 1e-6, 2 * self.asymmetric_alpha)
        ...
        while start_col_block < end_col_local:
            ...
            for col_index_in_block in range(this_block_size):
                ...
                if self.normalize:
                    ...
                    if self.asymmetric_cosine:
                        denominator = sum_of_squared_to_alpha[columnIndex] * \
                            sum_of_squared_to_1_minus_alpha + self.shrink + 1e-6
                    else:
                        denominator = sum_of_squared[columnIndex] * \
                            sum_of_squared + self.shrink + 1e-6
                ...
                # Apply the specific denominator for Tanimoto
                    ...
                    denominator = sum_of_squared[columnIndex] + \
                        sum_of_squared - this_column_weights + self.shrink + 1e-6
                    this_column_weights = np.multiply(
                        this_column_weights, 1 / denominator)
                    ...
                    denominator = this_column_weights + \
                        ...
                        (sum_of_squared - this_column_weights) * \
                        self.tversky_beta + self.shrink + 1e-6
                    this_column_weights = np.multiply(
                        this_column_weights, 1 / denominator)
                ...
                relevant_items_partition = np.argpartition(
                    -this_column_weights, self.topK - 1, axis=0)[0:self.topK]
                ...
                similarity_builder.add_data_lists(row_list_to_add=relevant_items_partition,
                                                  col_list_to_add=np.ones(
                                                      len(relevant_items_partition), dtype=np.int) * columnIndex,
            ...
            if time.time() - start_time_print_batch >= 300 or end_col_block == end_col_local:
                column_per_sec = processed_items / \
