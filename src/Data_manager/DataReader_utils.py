...
import pandas as pd
from src.Data_manager.IncrementalSparseMatrix import IncrementalSparseMatrix, IncrementalSparseMatrix_FilterIDs
import numpy as np
import time
import sys
import os
...
def split_big_CSR_in_columns(sparse_matrix_to_split, num_split=2):
    ...
    assert num_split >= 1 and num_split <= sparse_matrix_to_split.shape[1], "split_big_CSR_in_columns: num_split parameter not valid, value must be between 1 and {}, provided was {}".format(
    ...
    for num_current_split in range(num_split):
        ...
        print("split_big_CSR_in_columns: Split {}, columns: {}-{}".format(
            num_current_split, start_col, end_col))

        sparse_matrix_split_list.append(
            sparse_matrix_to_split[:, start_col:end_col])
...
def remove_Dataframe_duplicates(dataframe, unique_values_in_columns=['UserID', 'ItemID'], keep_highest_value_in_col="timestamp"):
    ...
    dataframe.sort_values(by=sort_by, ascending=True,
                          inplace=True, kind="quicksort", na_position="first")
    dataframe.drop_duplicates(unique_values_in_columns,
                              keep='last', inplace=True)

    n_data_points = len(dataframe[unique_values_in_columns[0]].values)

    n_unique_data_points = dataframe.drop_duplicates(
        unique_values_in_columns, keep='first', inplace=False).shape[0]
    ...
    if contains_duplicates_flag:
        if remove_duplicates:
            ...
            assert num_unique_user_item_ids == len(
                user_id_list), "load_CSV_into_SparseBuilder: duplicate (user, item) values found"
...
def load_CSV_into_SparseBuilder(filePath, header=False, separator="::", timestamp=False, remove_duplicates=False,
    ...
    if preinitialized_row_mapper is not None or preinitialized_col_mapper is not None:
        ...
        URM_all_builder = IncrementalSparseMatrix(
            auto_create_col_mapper=create_mapper, auto_create_row_mapper=create_mapper)
    ...
    if contains_duplicates_flag:
        if remove_duplicates:
            ...
            df_original.sort_values(
                by=sort_by, ascending=True, inplace=True, kind="quicksort", na_position="first")
            df_original.drop_duplicates(
                ["userId", "itemId"], keep='last', inplace=True)
    ...
    URM_all_builder.add_data_lists(
...
def remove_features(ICM, min_occurrence=5, max_percentage_occurrence=0.30, reconcile_mapper=None):
    ...
    feature_mask = np.logical_and(
        numOccurrences >= min_occurrence, numOccurrences <= n_items * max_percentage_occurrence)

    ICM = ICM[:, feature_mask]

    deletedFeatures = np.arange(0, len(feature_mask))[
        np.logical_not(feature_mask)]
    ...
    print("RemoveFeatures: removed {} features with less then {} occurrences, removed {} features with more than {} occurrencies".format(
        ...
        sum(numOccurrences > n_items *
            max_percentage_occurrence), int(n_items * max_percentage_occurrence)
    ))
...
def reconcile_mapper_with_removed_tokens(key_to_value_dict, values_to_remove):
    ...
    assert len(set(key_to_value_dict.values())) == len(
        key_to_value_dict), "mapper_dict values do not have a 1-to-1 correspondance with the key"
    ...
    mapper_values_array = np.ones(
        max(key_to_value_dict.values())+1, dtype=np.int) * -np.inf
...
def download_from_URL(URL, folder_path, file_name):
    ...
    try:
        ...
        urlretrieve(URL, folder_path + file_name,
                    reporthook=urllretrieve_reporthook)
