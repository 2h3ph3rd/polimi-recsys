...
def create_result_multiindex_dataframe(n_cases, result_df):
    ...
    cases_cutoff_multiindex = pd.MultiIndex.from_product(
        [np.arange(n_cases), result_df.index])
...
def add_result_to_multiindex_dataframe(destination_result_df, new_result_df, position):
    ...
    for index in new_result_df.index:
        destination_result_df.loc[position,
                                  index] = new_result_df.loc[index].copy()
...
class SearchInputRecommenderArgs(object):
    ...
    def __init__(self,
        ...
        assert isinstance(CONSTRUCTOR_POSITIONAL_ARGS,
                          list), "CONSTRUCTOR_POSITIONAL_ARGS must be a list"
        assert isinstance(CONSTRUCTOR_KEYWORD_ARGS,
                          dict), "CONSTRUCTOR_KEYWORD_ARGS must be a dict"

        assert isinstance(FIT_POSITIONAL_ARGS,
                          list), "FIT_POSITIONAL_ARGS must be a list"
        assert isinstance(FIT_KEYWORD_ARGS,
                          dict), "FIT_KEYWORD_ARGS must be a dict"

        assert isinstance(EARLYSTOPPING_KEYWORD_ARGS,
                          dict), "EARLYSTOPPING_KEYWORD_ARGS must be a dict"
...
def get_result_string_prettyprint(result_series_single_cutoff, n_decimals=7):
    ...
    for metric, value in result_series_single_cutoff.items():
        output_str += "{}: {:.{n_decimals}f}, ".format(
            metric, value, n_decimals=n_decimals)
...
class SearchAbstractClass(object):
    ...
    def _set_search_attributes(self, recommender_input_args,
        ...
        if save_model not in self._SAVE_MODEL_VALUES:
            raise ValueError("{}: argument save_model must be in '{}', provided was '{}'.".format(
                self.ALGORITHM_NAME, self._SAVE_MODEL_VALUES, save_model))

        if evaluate_on_test not in self._EVALUATE_ON_TEST_VALUES:
            raise ValueError("{}: argument evaluate_on_test must be in '{}', provided was '{}'.".format(
                self.ALGORITHM_NAME, self._EVALUATE_ON_TEST_VALUES, evaluate_on_test))
        ...
        self.log_file = open(self.output_folder_path + self.output_file_name_root +
                             "_{}.txt".format(self.ALGORITHM_NAME), "a")

        if save_model == "last" and recommender_input_args_last_test is None:
            self._write_log("{}: argument save_model is 'last' but no recommender_input_args_last_test provided, saving best model on train data alone.".format(
                self.ALGORITHM_NAME))
            save_model = "best"
        ...
        self._init_metadata_dict(
            n_cases=n_cases, hyperparameter_names=hyperparameter_names)
    ...
    def _evaluate_on_validation(self, current_fit_hyperparameters, was_already_evaluated_flag, was_already_evaluated_index):
        ...
        if not was_already_evaluated_flag:
            ...
            for key in current_fit_hyperparameters.keys():
                self.metadata_dict["hyperparameters_df"].loc[self.model_counter,
                                                             key] = current_fit_hyperparameters[key]
            ...
            if isinstance(recommender_instance, Incremental_Training_Early_Stopping):
                for epoch_key, epoch_value in recommender_instance.get_early_stopping_final_epochs_dict().items():
                    self.metadata_dict["hyperparameters_df"].loc[self.model_counter, epoch_key] = int(
                        epoch_value)

                # This is to ensure backward compatibility
                if "result_on_earlystopping_df" not in self.metadata_dict:
                    self.metadata_dict["result_on_earlystopping_df"] = pd.DataFrame(dtype=object) if issubclass(
                        self.recommender_class, Incremental_Training_Early_Stopping) else None

                # Add the data from all validation steps
                if self.metadata_dict["result_on_earlystopping_df"] is not None:
                    earlystopping_df_multiindex = pd.concat(
                        {self.model_counter: recommender_instance.get_validation_summary_table()}, names=['model_counter'])
                    self.metadata_dict["result_on_earlystopping_df"] = pd.concat(
                        [self.metadata_dict["result_on_earlystopping_df"], earlystopping_df_multiindex])

        else:
            # If it was already evaluated load the data
            recommender_instance = None

            self.metadata_dict["hyperparameters_df"].loc[self.model_counter] = self.metadata_dict[
                "hyperparameters_df"].loc[was_already_evaluated_index].copy()
        ...
        if self.metadata_dict["result_on_validation_df"] is None:
            # The dataframe will have the case number and cutoff as index, the metric name as column
            self.metadata_dict["result_on_validation_df"] = create_result_multiindex_dataframe(
                self.n_cases, result_df)

        add_result_to_multiindex_dataframe(
            self.metadata_dict["result_on_validation_df"], result_df, self.model_counter)
        ...
        self.metadata_dict["time_on_train_avg"] = self.metadata_dict["time_df"]["train"].mean(
            axis=0, skipna=True)
        self.metadata_dict["time_on_train_total"] = self.metadata_dict["time_df"]["train"].sum(
            axis=0, skipna=True)
        self.metadata_dict["time_on_validation_avg"] = self.metadata_dict["time_df"]["validation"].mean(
            axis=0, skipna=True)
        self.metadata_dict["time_on_validation_total"] = self.metadata_dict["time_df"]["validation"].sum(
            axis=0, skipna=True)
    ...
    def _evaluate_on_test(self, recommender_instance, current_fit_hyperparameters_dict,
        ...
        if self.metadata_dict["result_on_test_df"] is None:
            # The dataframe will have the case number and cutoff as index, the metric name as column
            self.metadata_dict["result_on_test_df"] = create_result_multiindex_dataframe(
                self.n_cases, result_df_test)

        add_result_to_multiindex_dataframe(
            self.metadata_dict["result_on_test_df"], result_df_test, self.model_counter)

        self.metadata_dict["time_df"].loc[self.model_counter,
                                          "test"] = evaluation_test_time
        self.metadata_dict["time_on_test_avg"] = self.metadata_dict["time_df"]["test"].mean(
            axis=0, skipna=True)
        self.metadata_dict["time_on_test_total"] = self.metadata_dict["time_df"]["test"].sum(
            axis=0, skipna=True)
    ...
    def _evaluate_on_test_with_data_last(self):
        ...
        if self.resume_from_saved and self.metadata_dict["result_on_last"] is not None:
            self._print("{}: Resuming '{}'... Result on last already available.".format(
                self.ALGORITHM_NAME, self.output_file_name_root))
            return

        self._print("{}: Evaluation with constructor data for final test. Using best config: {}".format(
            self.ALGORITHM_NAME, self.metadata_dict["hyperparameters_best"]))

        # Use the hyperparameters that have been saved
        assert self.metadata_dict["hyperparameters_best"] is not None, "{}: Best hyperparameters not available, the search might have failed.".format(
            self.ALGORITHM_NAME)
        ...
        if self.evaluate_on_test in ["all", "best", "last"]:
            ...
            self.metadata_dict["time_on_last_df"].loc[0,
                                                      "test"] = evaluation_test_time
        ...
        if self.save_model in ["all", "best", "last"]:
            self._print("{}: Saving model in {}\n".format(
                self.ALGORITHM_NAME, self.output_folder_path + self.output_file_name_root))
    ...
    def _objective_function(self, current_fit_hyperparameters_dict):
        ...
        try:
            ...
            result_df, recommender_instance = self._evaluate_on_validation(
                current_fit_hyperparameters_dict, was_already_evaluated_flag, was_already_evaluated_index)
            ...
            if self.save_model in ["all"] and not was_already_evaluated_flag:
                self._print("{}: Saving model in {}\n".format(
                    self.ALGORITHM_NAME, self.output_folder_path + self.output_file_name_root))
                recommender_instance.save_model(
                    self.output_folder_path, file_name=self.output_file_name_root + "_model_{}".format(self.model_counter))
            ...
            if new_best_config_found:
                ...
                self._write_log("{}: Config {} {}. Config: {} - results: {}\n".format(self.ALGORITHM_NAME,
                                                                                      self.model_counter,
                                                                                      "is suboptimal" if not was_already_evaluated_flag else "was already explored at index {}".format(
            ...
            if new_best_config_found:
                ...
                if self.save_model in ["all", "best"]:
                    self._print("{}: Saving model in {}\n".format(
                        self.ALGORITHM_NAME, self.output_folder_path + self.output_file_name_root))
                    recommender_instance.save_model(
                        self.output_folder_path, file_name=self.output_file_name_root + "_best_model")
        ...
        # Catch exception only if terminate_on_memory_error is True
            ...
            self._print("{}: Search for '{}' interrupted due to MemoryError.".format(
                self.ALGORITHM_NAME, self.metadata_dict["algorithm_name_recommender"]))
            return
