...
def json_not_serializable_handler(o):
    ...
    raise TypeError(
        "json_not_serializable_handler: object '{}' is not serializable.".format(type(o)))
...
class DataIO(object):
    ...
    def _get_temp_folder(self, file_name):
        ...
        current_temp_folder = "{}{}_{}_{}/".format(
            self.folder_path, self._DEFAULT_TEMP_FOLDER, os.getpid(), file_name)
    ...
    def _check_dict_key_type(self, dict_to_save):
        ...
        all_keys_are_str = all(isinstance(key, str)
        ...
        dict_to_save_key_str = {str(key): val for (
            key, val) in dict_to_save.items()}
        ...
        try:
            ...
            for attrib_name, attrib_data in data_dict_to_save.items():
                ...
                if isinstance(attrib_data, DataFrame):
                    ...
                    try:
                        _ = json.dumps(
                            attrib_data, default=json_not_serializable_handler)
                        ...
                        if isinstance(attrib_data, dict):
                            ...
                            dataIO.save_data(
                                file_name=attrib_name, data_dict_to_save=attrib_data)

                        else:
                            raise TypeError(
                                "Type not recognized for attribute: {}".format(attrib_name))
            ...
            with zipfile.ZipFile(self.folder_path + file_name + ".temp", 'w', compression=zipfile.ZIP_DEFLATED) as myzip:
                for file_to_compress in os.listdir(current_temp_folder):
                    myzip.write(current_temp_folder +
                                file_to_compress, arcname=file_to_compress)
            ...
            os.replace(self.folder_path + file_name +
                       ".temp", self.folder_path + file_name)
    ...
    def load_data(self, file_name):
        ...
        try:
            ...
            try:
                data_format = dataFile.extract(
                    ".data_format.json", path=current_temp_folder)
            ...
            for file_name in dataFile.namelist():
                ...
                decompressed_file_path = dataFile.extract(
                    file_name, path=current_temp_folder)
                file_extension = file_name.split(".")[-1]
                attrib_name = file_name[:-len(file_extension)-1]

                if file_extension == "csv":
                    # Compatibility with previous version
                    attrib_data = pd.read_csv(
                        decompressed_file_path, index_col=False)

                elif file_extension == "h5":
                    attrib_data = pd.read_hdf(
                        decompressed_file_path, key=None, mode='r')
                    ...
                    # allow_pickle is FALSE to prevent using pickle and ensure portability
                    attrib_data = np.load(
                        decompressed_file_path, allow_pickle=False)
                    ...
                    raise Exception("Attribute type not recognized for: '{}' of class: '{}'".format(
...
class MyTestCase(unittest.TestCase):
    ...
    def test_save_and_load(self):
        ...
        dataIO.save_data(file_name="test_DataIO",
                         data_dict_to_save=original_data_dict)
        ...
        self.assertTrue((original_data_dict['dataframe'].dtypes == loaded_data_dict['dataframe'].dtypes).all(
        )), "Datatypes are different"

        # Check column with different data types: float, int, string, None
        self.assertTrue(type(original_data_dict['dataframe'].loc[0, "I am a mess"]) == type(
            loaded_data_dict['dataframe'].loc[0, "I am a mess"])), "Datatypes are different"
        self.assertTrue(type(original_data_dict['dataframe'].loc[1, "I am a mess"]) == type(
            loaded_data_dict['dataframe'].loc[1, "I am a mess"])), "Datatypes are different"
        self.assertTrue(type(original_data_dict['dataframe'].loc[2, "I am a mess"]) == type(
            loaded_data_dict['dataframe'].loc[2, "I am a mess"])), "Datatypes are different"
        self.assertTrue(type(original_data_dict['dataframe'].loc[3, "I am a mess"]) == type(
            loaded_data_dict['dataframe'].loc[3, "I am a mess"])), "Datatypes are different"

        # Check various data types: scipy sparse, string, list...
        self.assertTrue(np.array_equal(original_data_dict['sps_random'].toarray(
        ), loaded_data_dict['sps_random'].toarray()))
        self.assertTrue(
            original_data_dict['result_folder_path'] == loaded_data_dict['result_folder_path'])
        self.assertTrue(original_data_dict['cutoff_list_validation']
                        == loaded_data_dict['cutoff_list_validation'])

        self.assertTrue(original_data_dict['dataframe'].equals(
            loaded_data_dict['dataframe']))
        self.assertTrue(original_data_dict['multiindex_df_row'].equals(
            loaded_data_dict['multiindex_df_row']))
        self.assertTrue(original_data_dict['multiindex_df_col'].equals(
            loaded_data_dict['multiindex_df_col']))

        # Check content of nested dictionary
        self.assertEqual(original_data_dict['nested_dict'].keys(
        ), loaded_data_dict['nested_dict'].keys())
        self.assertTrue(
            original_data_dict['nested_dict']["A"] == loaded_data_dict['nested_dict']["A"])
        self.assertTrue(np.array_equal(original_data_dict['nested_dict']["B"].toarray(
        ), loaded_data_dict['nested_dict']["B"].toarray()))
