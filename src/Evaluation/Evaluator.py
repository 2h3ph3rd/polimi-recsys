...
from src.Evaluation.metrics import precision, precision_recall_min_denominator, recall, MAP, MAP_MIN_DEN, MRR, HIT_RATE, ndcg, arhr_all_hits, \
    ...
    for cutoff in cutoff_list:
        ...
        for metric in EvaluatorMetrics:
            if metric == EvaluatorMetrics.COVERAGE_ITEM:
                cutoff_dict[metric.value] = Coverage_Item(
                    n_items, ignore_items)

            elif metric == EvaluatorMetrics.COVERAGE_ITEM_HIT:
                cutoff_dict[metric.value] = Coverage_Item_HIT(
                    n_items, ignore_items)
                ...
                cutoff_dict[metric.value] = Gini_Diversity(
                    n_items, ignore_items)

            elif metric == EvaluatorMetrics.SHANNON_ENTROPY:
                cutoff_dict[metric.value] = Shannon_Entropy(
                    n_items, ignore_items)

            elif metric == EvaluatorMetrics.COVERAGE_USER:
                cutoff_dict[metric.value] = Coverage_User(
                    n_users, ignore_users)

            elif metric == EvaluatorMetrics.COVERAGE_USER_HIT:
                cutoff_dict[metric.value] = Coverage_User_HIT(
                    n_users, ignore_users)
                ...
                cutoff_dict[metric.value] = Diversity_MeanInterList(
                    n_items, cutoff)
                ...
                cutoff_dict[metric.value] = Ratio_Diversity_Gini(
                    URM_train, ignore_items)

            elif metric == EvaluatorMetrics.RATIO_DIVERSITY_HERFINDAHL:
                cutoff_dict[metric.value] = Ratio_Diversity_Herfindahl(
                    URM_train, ignore_items)

            elif metric == EvaluatorMetrics.RATIO_SHANNON_ENTROPY:
                cutoff_dict[metric.value] = Ratio_Shannon_Entropy(
                    URM_train, ignore_items)
                ...
                if diversity_similarity_object is not None:
                    cutoff_dict[metric.value] = copy.deepcopy(
    ...
    for cutoff in results_run_df.index:
        ...
        for metric in results_run_df.columns:
            output_str += "{}: {:.{n_decimals}f}, ".format(
                metric, results_run_df.loc[cutoff, metric], n_decimals=n_decimals)
...
class Evaluator(object):
    ...
    def __init__(self, URM_test, cutoff_list, min_ratings_per_user=1, exclude_seen=True,
        ...
        self.URM_test, users_to_evaluate_mask = _prune_users(
            URM_test, self.ignore_items_ID, min_ratings_per_user)
        ...
        if ignore_users is not None:
            ...
            self.users_to_evaluate = set(
                self.users_to_evaluate) - set(ignore_users)
        else:
    ...
    def evaluateRecommender(self, recommender_object):
        ...
        results_dict = self._run_evaluation_on_selected_users(
            recommender_object, self.users_to_evaluate)
            ...
            for cutoff in self.cutoff_list:
                ...
                for key in results_current_cutoff.keys():
                    ...
                    if isinstance(value, _Metrics_Object):
                        ...
                        results_current_cutoff[key] = value / \
    ...
    def get_user_relevant_items(self, user_id):
        ...
        assert self.URM_test.getformat(
        ) == "csr", "Evaluator_Base_Class: URM_test is not CSR, this will cause errors in getting relevant items"
    ...
    def get_user_test_ratings(self, user_id):
        ...
        assert self.URM_test.getformat(
        ) == "csr", "Evaluator_Base_Class: URM_test is not CSR, this will cause errors in relevant items ratings"
        ...
        for batch_user_index in range(len(recommended_items_batch_list)):
            ...
            for cutoff in self.cutoff_list:
                ...
                results_current_cutoff[EvaluatorMetrics.NDCG.value] += ndcg(
                    recommended_items_current_cutoff, relevant_items, relevance=self.get_user_test_ratings(test_user), at=cutoff)
                results_current_cutoff[EvaluatorMetrics.ARHR.value] += arhr_all_hits(
                    is_relevant_current_cutoff)

                results_current_cutoff[EvaluatorMetrics.MRR.value].add_recommendations(
                    is_relevant_current_cutoff)
                results_current_cutoff[EvaluatorMetrics.MAP.value].add_recommendations(
                    is_relevant_current_cutoff, relevant_items)
                results_current_cutoff[EvaluatorMetrics.MAP_MIN_DEN.value].add_recommendations(
                    is_relevant_current_cutoff, relevant_items)
                results_current_cutoff[EvaluatorMetrics.HIT_RATE.value].add_recommendations(
                    is_relevant_current_cutoff)
                ...
                results_current_cutoff[EvaluatorMetrics.COVERAGE_ITEM.value].add_recommendations(
                    recommended_items_current_cutoff)
                results_current_cutoff[EvaluatorMetrics.COVERAGE_ITEM_HIT.value].add_recommendations(
                ...
                results_current_cutoff[EvaluatorMetrics.COVERAGE_USER_HIT.value].add_recommendations(
                    is_relevant_current_cutoff, test_user)
                ...
                results_current_cutoff[EvaluatorMetrics.RATIO_SHANNON_ENTROPY.value].add_recommendations(
                    recommended_items_current_cutoff)
                results_current_cutoff[EvaluatorMetrics.RATIO_DIVERSITY_HERFINDAHL.value].add_recommendations(
                    recommended_items_current_cutoff)
                results_current_cutoff[EvaluatorMetrics.RATIO_DIVERSITY_GINI.value].add_recommendations(
                    recommended_items_current_cutoff)
                results_current_cutoff[EvaluatorMetrics.RATIO_NOVELTY.value].add_recommendations(
                    recommended_items_current_cutoff)
                results_current_cutoff[EvaluatorMetrics.RATIO_AVERAGE_POPULARITY.value].add_recommendations(
                    recommended_items_current_cutoff)
...
class EvaluatorHoldout(Evaluator):
    ...
    def _run_evaluation_on_selected_users(self, recommender_object, users_to_evaluate, block_size=None):
        ...
        if block_size is None:
            ...
            block_size = min(
                [1000, int(4*1e9*8/64/self.n_items), len(users_to_evaluate)])
        ...
        while user_batch_start < len(users_to_evaluate):
            ...
            test_user_batch_array = np.array(
                users_to_evaluate[user_batch_start:user_batch_end])
