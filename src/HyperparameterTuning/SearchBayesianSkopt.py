...
def _extend_dataframe(initial_dataframe, new_rows):
    ...
    if isinstance(initial_dataframe.index, pd.MultiIndex):
        ...
        new_df = pd.DataFrame(
            columns=initial_dataframe.columns, index=cases_cutoff_multiindex)
    else:
        new_df = pd.DataFrame(columns=initial_dataframe.columns, index=range(
            len(initial_dataframe), len(initial_dataframe)+new_rows))
...
class SearchBayesianSkopt(SearchAbstractClass):
    ...
    def __init__(self, recommender_class, evaluator_validation=None, evaluator_test=None, verbose=True):
        ...
        assert evaluator_validation is not None, "{}: evaluator_validation must be provided".format(
            self.ALGORITHM_NAME)
    ...
    def _set_skopt_params(self,
        ...
        self.random_state = int(os.getpid() + time.time()
    ...
    def _resume_from_saved(self):
        ...
        try:
            ...
            n_cases_in_loaded_data = len(
                self.metadata_dict["hyperparameters_df"])
            if n_cases_in_loaded_data < self.n_cases:
                ...
                self._write_log("{}: Extending previous number of cases from {} to {}.\n".format(
                    self.ALGORITHM_NAME, n_cases_in_loaded_data, self.n_cases))
                ...
                for dataframe_name in ["hyperparameters_df", "time_df", "result_on_validation_df", "result_on_test_df"]:
                    self.metadata_dict[dataframe_name] = _extend_dataframe(
                        self.metadata_dict[dataframe_name], new_cases)
            ...
            self._write_log("{}: Resuming '{}' Failed, generic exception: {}.\n".format(
                self.ALGORITHM_NAME, self.output_file_name_root, str(e)))
            raise e
        ...
        assert hyperparameters_df[:self.model_counter].notna().any(axis=1).all(),\
            "{}: Resuming '{}' Failed due to inconsistent data, valid hyperparameter configurations are not contiguous at the beginning of the dataframe.".format(
                self.ALGORITHM_NAME, self.output_file_name_root)
        ...
        for index in range(self.model_counter):
            ...
            is_validation_valid = result_on_validation_df is not None and result_on_validation_df[self.metric_to_optimize].notna()[
                index].any()
            assert is_exception == is_validation_valid,\
                "{}: Resuming '{}' Failed due to inconsistent data. There cannot be both a valid result and an exception for the same case.".format(
                    self.ALGORITHM_NAME, self.output_file_name_root)

        if result_on_validation_df is not None:
            result_on_validation_df = result_on_validation_df.fillna(
                value=- self.INVALID_CONFIG_VALUE, inplace=False)
            result_on_validation_df = result_on_validation_df.loc[:self.model_counter-1]
            result_on_validation_list_input = (
                - result_on_validation_df[self.metric_to_optimize].loc[:, self.cutoff_to_optimize]).to_list()
        else:
            ...
            result_on_validation_list_input = [
                + self.INVALID_CONFIG_VALUE]*self.model_counter

        assert len(hyperparameters_list_input) == len(result_on_validation_list_input), \
            "{}: Resuming '{}' Failed due to inconsistent data, there is a different number of hyperparameters and results.".format(
                self.ALGORITHM_NAME, self.output_file_name_root)

        self._print("{}: Resuming '{}'... Loaded {} configurations.".format(
    ...
    def _was_already_evaluated_check(self, current_fit_hyperparameters_dict):
        ...
        current_fit_hyperpar_series = pd.Series(
            current_fit_hyperparameters_dict)
        hyperparameters_df, current_fit_hyperpar_series = hyperparameters_df.align(
            current_fit_hyperpar_series, axis=1, copy=False)

        is_equal = (hyperparameters_df ==
                    current_fit_hyperpar_series).all(axis=1)
    ...
    def search(self, recommender_input_args,
        ...
        if self.max_total_time is not None:
            total_time_value, total_time_unit = seconds_to_biggest_unit(
                self.max_total_time)
            self._print("{}: The search has a maximum allotted time of {:.2f} {}".format(
                self.ALGORITHM_NAME, total_time_value, total_time_unit))
        ...
        for name, hyperparam in hyperparameter_search_space.items():
            ...
            if any(isinstance(hyperparam, sko_type) for sko_type in skopt_types):
                ...
                raise ValueError("{}: Unexpected hyperparameter type: {} - {}".format(
        ...
        try:
            ...
            if self.n_calls - self.model_counter > 0:
                ...
                self.result = gp_minimize(self._objective_function_list_input,
                                          ...
                                          base_estimator=None,
                                          n_calls=max(
                                              0, self.n_calls - self.model_counter),
    ...
    def _objective_function_list_input(self, current_fit_hyperparameters_list_of_values):
        ...
        total_current_time = self.metadata_dict["time_on_train_total"] + \
            self.metadata_dict["time_on_validation_total"]
        ...
        if self.max_total_time is not None:
            ...
            if self.recommender_input_args_last_test is None and total_current_time > self.max_total_time:
                ...
                raise TimeoutError(self.max_total_time,
                                   total_current_time + estimated_last_time)

        current_fit_hyperparameters_dict = dict(
            zip(self.hyperparams_names, current_fit_hyperparameters_list_of_values))
