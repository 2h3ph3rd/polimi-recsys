...
class GeneralizedMergedHybridRecommender(BaseRecommender):
    ...
    def __init__(
        ...
        for recommender in recommenders:
            self.RECOMMENDER_NAME = self.RECOMMENDER_NAME + \
                recommender.RECOMMENDER_NAME[:-11]
    ...
    def _compute_item_score(self, user_id_array, items_to_compute=None):
        result = self.alphas[0]*self.recommenders[0]._compute_item_score(
            user_id_array, items_to_compute)
        for index in range(1, len(self.alphas)):
            result = result + \
                self.alphas[index]*self.recommenders[index]._compute_item_score(
                    user_id_array, items_to_compute)
        return result
