...
class HP3_Similarity_Cython(BaseItemCBFRecommender, BaseItemSimilarityMatrixRecommender, Incremental_Training_Early_Stopping):
    ...
    def fit(self, show_max_performance=False,
        ...
        if init_value <= 0:
            ...
            print(self.RECOMMENDER_NAME +
                  ": Invalid init value, using default (" + str(self.INIT_VALUE) + ")")
        ...
        weights_initialization_D = np.ones(
            self.n_features, dtype=np.float64) * init_value
    ...
    def compute_W_sparse(self, model_to_use="best"):
        ...
        self.W_sparse = normalize(sps.csr_matrix((data, indices, indptr), shape=(
            self.n_items, self.n_items)), norm="l1", axis=1)
