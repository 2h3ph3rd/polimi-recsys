...
def split_train_leave_k_out_user_wise(URM, k_out=1, use_validation_set=True, leave_random_out=True):
    ...
    for user_id in range(n_users):
        ...
        if leave_random_out:
            ...
            user_interaction_data = URM.data[start_user_position:
                                             end_user_position][indices_to_suffle]
            ...
            user_interaction_data = URM.data[start_user_position:
                                             end_user_position][sort_interaction_index]
        ...
        URM_test_builder.add_data_lists(
            [user_id]*len(user_interaction_items_test), user_interaction_items_test, user_interaction_data_test)
        ...
        if use_validation_set:
            ...
            URM_validation_builder.add_data_lists(
                [user_id]*k_out, user_interaction_items_validation, user_interaction_data_validation)
        ...
        URM_train_builder.add_data_lists(
            [user_id]*len(user_interaction_items_train), user_interaction_items_train, user_interaction_data_train)
    ...
    if use_validation_set:
        ...
        if user_no_item_validation != 0:
            print("Warning: {} ({:.2f} %) of {} users have no Validation items".format(
                user_no_item_validation, user_no_item_validation/n_users*100, n_users))
