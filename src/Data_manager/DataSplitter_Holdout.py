...
from src.Data_manager.DataSplitter import DataSplitter as _DataSplitter
from src.Data_manager.DataReader import DataReader as _DataReader
from src.Data_manager.split_functions.split_train_validation_random_holdout import split_train_in_two_percentage_user_wise, split_train_in_two_percentage_global_sample
from src.Data_manager.DataReader_utils import compute_density, reconcile_mapper_with_removed_tokens
from src.Data_manager.data_consistency_check import assert_disjoint_matrices, assert_URM_ICM_mapper_consistency
...
class DataSplitter_Holdout(_DataSplitter):
    ...
    def __init__(self, dataReader_object: _DataReader,
        ...
        if split_interaction_quota_list is None:
            ...
            self._print("input_split_item_quota_list not provided, using default '{}'".format(
                split_interaction_quota_list))

        assert len(split_interaction_quota_list) == 3, "{}: input_split_item_quota_list must contain 3 values: Train, Validation, Test".format(
            self.DATA_SPLITTER_NAME)
        assert all(split_quota >= 0.0 and split_quota <=
                   100 for split_quota in split_interaction_quota_list), "{}: input_split_item_quota_list must contain values between 0 and 100".format(self.DATA_SPLITTER_NAME)
        assert sum(split_interaction_quota_list) == 100, "{}: input_split_item_quota_list must be a probability distribution and sum to 100, current data sums to '{}'".format(
            self.DATA_SPLITTER_NAME, sum(split_interaction_quota_list))
        ...
        super(DataSplitter_Holdout, self).__init__(dataReader_object,
                                                   forbid_new_split=forbid_new_split, force_new_split=force_new_split)
    ...
    def get_statistics_URM(self):
        ...
        statistics_string = "DataReader: {}\n" \
                            ...
                            "\tTest \t\tquota {:.2f} ({:.2f}), \tinteractions {}, \tdensity {:.2E}\n".format(
                                ...
                                n_items,
                                n_users,
                                self.input_split_interaction_quota_list[0], self.actual_split_interaction_quota_list[0], self.SPLIT_URM_DICT["URM_train"].nnz, compute_density(
                                    self.SPLIT_URM_DICT["URM_train"]),
                                self.input_split_interaction_quota_list[1], self.actual_split_interaction_quota_list[1], self.SPLIT_URM_DICT["URM_validation"].nnz, compute_density(
                                    self.SPLIT_URM_DICT["URM_validation"]),
                                self.input_split_interaction_quota_list[2], self.actual_split_interaction_quota_list[
                                    2], self.SPLIT_URM_DICT["URM_test"].nnz, compute_density(self.SPLIT_URM_DICT["URM_test"]),
                            )
    ...
    def _assert_is_initialized(self):
        assert self.SPLIT_URM_DICT is not None, "{}: Unable to load data split. The split has not been generated yet, call the load_data function to do so.".format(
            self.DATA_SPLITTER_NAME)
    ...
    def _split_data_from_original_dataset(self, save_folder_path):
        ...
        if not self.allow_cold_users:
            ...
            if user_to_remove.sum() > 0:
                ...
                self._print("Removing {} ({:.2f} %) of {} users because they have no interactions in train data.".format(
    ...
    def _compute_real_split_interaction_quota(self):
        ...
        self.actual_split_interaction_quota_list = [
            None] * len(self.input_split_interaction_quota_list)
    ...
    def _save_split(self, save_folder_path):
        ...
        if save_folder_path:
            ...
            name_suffix = "_{}_{}".format(
                allow_cold_users_suffix, user_wise_string)
    ...
    def _load_previously_built_split_and_attributes(self, save_folder_path):
        ...
        split_parameters_dict = dataIO.load_data(
            file_name="split_parameters" + name_suffix)
        ...
        self.SPLIT_URM_DICT = dataIO.load_data(
            file_name="split_URM" + name_suffix)

        if len(self.dataReader_object.get_loaded_ICM_names()) > 0:
            self.SPLIT_ICM_DICT = dataIO.load_data(
                file_name="split_ICM" + name_suffix)
        ...
        if len(self.dataReader_object.get_loaded_UCM_names()) > 0:
            self.SPLIT_UCM_DICT = dataIO.load_data(
                file_name="split_UCM" + name_suffix)
    ...
    def _verify_data_consistency(self):
        ...
        assert len(self.SPLIT_URM_DICT) == len(self._SPLIT_URM_NAME_LIST),\
            print_preamble + "The available URM are not as many as they are supposed to be. URMs are {}, expected URMs are {}".format(
                len(self.SPLIT_URM_DICT), len(self._SPLIT_URM_NAME_LIST))

        assert all(URM_name in self.SPLIT_URM_DICT for URM_name in self._SPLIT_URM_NAME_LIST), print_preamble + \
            "Not all URMs have been created"
        assert all(URM_name in self._SPLIT_URM_NAME_LIST for URM_name in self.SPLIT_URM_DICT.keys(
        ...
        assert self.SPLIT_URM_DICT["URM_train"].nnz != 0, print_preamble + \
            "Number of interactions in URM Train is 0"
        assert self.SPLIT_URM_DICT["URM_test"].nnz != 0, print_preamble + \
            "Number of interactions in URM Test is 0"
        ...
        assert (self.SPLIT_URM_DICT["URM_validation"].nnz == 0 and self.input_split_interaction_quota_list[1] == 0.0)\
            or (self.SPLIT_URM_DICT["URM_validation"].nnz != 0 and self.input_split_interaction_quota_list[1] > 0.0), print_preamble + "Number of interactions in Validation is 0"
        ...
        assert_URM_ICM_mapper_consistency(URM_DICT=self.SPLIT_URM_DICT,
                                          user_original_ID_to_index=self.SPLIT_GLOBAL_MAPPER_DICT[
                                              "user_original_ID_to_index"],
                                          item_original_ID_to_index=self.SPLIT_GLOBAL_MAPPER_DICT[
