...
from src.Recommenders.Recommender_utils import similarityMatrixTopK
...
class MyTestCase(unittest.TestCase):
    ...
    def test_cosine_similarity_dense(self):
        ...
        cosine_similarity = Cosine_Similarity_Cython(
            data_matrix, topK=TopK, normalize=False)
        W_dense_Cython = cosine_similarity.compute_similarity()

        cosine_similarity = Compute_Similarity_Python(
            data_matrix, topK=TopK, normalize=False)
        W_dense_Python = cosine_similarity.compute_similarity()

        cosine_similarity = Cosine_Similarity_Parallel(
            data_matrix, topK=TopK, normalize=False)
    ...
    def test_cosine_similarity_dense_row_weighted(self):
        ...
        cosine_similarity = Cosine_Similarity_Cython(
            data_matrix.T, topK=TopK, normalize=False, row_weights=row_weights)
        W_dense_Cython = cosine_similarity.compute_similarity()
        ...
        W_dense_mul = data_matrix.dot(
            sps.diags(row_weights)).dot(data_matrix.T).toarray()
        W_dense_mul[np.arange(W_dense_mul.shape[0]),
                    np.arange(W_dense_mul.shape[0])] = 0.0
    ...
    def test_cosine_similarity_dense_external_cfr(self):
        ...
        cosine_similarity = Cosine_Similarity_Cython(
            data_matrix, topK=TopK, normalize=True, shrink=shrink)
        W_dense_Cython = cosine_similarity.compute_similarity()
        ...
        W_dense_sklearn[np.arange(W_dense_sklearn.shape[0]), np.arange(
            W_dense_sklearn.shape[0])] = 0.0

        assert np.allclose(W_dense_Cython, W_dense_sklearn,
                           atol=1e-4), "W_dense_Cython Cosine not matching Sklearn control"
        assert np.allclose(W_dense_Python, W_dense_sklearn,
                           atol=1e-4), "W_dense_Python Cosine not matching Sklearn control"
        assert np.allclose(W_dense_Parallel, W_dense_sklearn,
        ...
        assert np.allclose(W_dense_Cython, W_dense_Scipy,
                           atol=1e-4), "W_dense_Cython Jaccard not matching Scipy control"
        assert np.allclose(W_dense_Python, W_dense_Scipy,
                           atol=1e-4), "W_dense_Python Jaccard not matching Scipy control"
        assert np.allclose(W_dense_Parallel, W_dense_Scipy,
    ...
    def test_cosine_similarity_dense_normalize(self):
        ...
        W_dense_denominator = np.multiply(
            W_dense_denominator, W_dense_denominator.T) + shrink
    ...
    def test_cosine_similarity_dense_adjusted(self):
        ...
        for row in range(data_matrix.shape[0]):
            ...
            data_matrix[row,
                        :][nonzeroMask] -= np.mean(data_matrix[row, :][nonzeroMask])

        W_dense_denominator = np.matlib.repmat(
            (data_matrix**2).sum(axis=0), data_matrix.shape[1], 1)
        W_dense_denominator = np.sqrt(W_dense_denominator)
        W_dense_denominator = np.multiply(
            W_dense_denominator, W_dense_denominator.T) + shrink
        ...
        W_dense_mul = data_matrix.T.dot(data_matrix)
        W_dense_mul[W_dense_denominator >
                    0] /= W_dense_denominator[W_dense_denominator > 0]
    ...
    def test_cosine_similarity_dense_pearson(self):
        ...
        for col in range(data_matrix.shape[1]):
            ...
            data_matrix[:,
                        col][nonzeroMask] -= np.mean(data_matrix[:, col][nonzeroMask])
    ...
    def test_cosine_similarity_dense_jaccard(self):
        ...
        W_dense_denominator = np.matlib.repmat(
            (data_matrix**2).sum(axis=0), data_matrix.shape[1], 1)
        W_dense_denominator = W_dense_denominator + \
            W_dense_denominator.T - W_dense_mul + shrink
