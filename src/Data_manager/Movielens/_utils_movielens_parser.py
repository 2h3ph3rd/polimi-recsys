...
def _loadICM_genres_years(genres_path, header=True, separator=',', genresSeparator="|"):
    ...
    ICM_genres_dataframe = pd.read_csv(filepath_or_buffer=genres_path, sep=separator, header=header, dtype={
                                       0: str, 1: str, 2: str}, engine='python')
    ICM_genres_dataframe.columns = ["ItemID", "Title", "GenreList"]

    ICM_years_dataframe = ICM_genres_dataframe.copy()
    ICM_years_dataframe["Year"] = ICM_years_dataframe["Title"].str.extract(
        pat='\(([0-9]{4})\)$')
...
def _loadURM(URM_path, header=None, separator=','):
    ...
    URM_all_dataframe = pd.read_csv(filepath_or_buffer=URM_path, sep=separator, header=header, dtype={
                                    0: str, 1: str, 2: float, 3: int}, engine='python')
    URM_all_dataframe.columns = [
        "UserID", "ItemID", "Interaction", "Timestamp"]

    URM_timestamp_dataframe = URM_all_dataframe.copy().drop(columns=[
        "Interaction"])
...
def _loadICM_tags(tags_path, header=True, separator=','):
    ...
    # Tags
    from src.Data_manager.TagPreprocessing import tagFilterAndStemming
