...
from src.Recommenders.Recommender_utils import get_unique_temp_folder
import os
import shutil
...
class BaseTempFolder(object):
    ...
    def __init__(self):
        ...
        self.DEFAULT_TEMP_FILE_FOLDER = './result_experiments/__Temp_{}_{}/'.format(
            self.RECOMMENDER_NAME, os.getpid())
    ...
    def _get_unique_temp_folder(self, input_temp_file_folder=None):
        ...
        if input_temp_file_folder is None:
            ...
            output_temp_file_folder = get_unique_temp_folder(
                self.DEFAULT_TEMP_FILE_FOLDER)
        else:
            print("{}: Using Temp folder '{}'".format(
                self.RECOMMENDER_NAME, input_temp_file_folder))
            self._use_default_temp_folder = False
            output_temp_file_folder = get_unique_temp_folder(
                input_temp_file_folder)
    ...
    def _clean_temp_folder(self, temp_file_folder):
        ...
        if self._use_default_temp_folder:
            print("{}: Cleaning temporary files from '{}'".format(
                self.RECOMMENDER_NAME, temp_file_folder))
            shutil.rmtree(temp_file_folder, ignore_errors=True)

        else:
            print("{}: Maintaining temporary files due to a custom temp folder being selected".format(
