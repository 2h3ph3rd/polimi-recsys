...
from src.Recommenders.Neural.architecture_utils import generate_autoencoder_architecture


from src.Recommenders.BaseRecommender import BaseRecommender
from src.Recommenders.BaseTempFolder import BaseTempFolder
...
class _MultDAE_original(object):
    ...
    def construct_placeholders(self):
        ...
        self.keep_prob_ph = tf.compat.v1.placeholder_with_default(
            1.0, shape=None)
...
class _MultVAE_original(_MultDAE_original):
    ...
    def construct_placeholders(self):
        ...
        self.is_training_ph = tf.compat.v1.placeholder_with_default(
            0., shape=None)
    ...
    def build_graph(self):
        ...
        # apply regularization to weights
        reg = tf.keras.regularizers.l2(self.lam)
        neg_ELBO = neg_ll + self.anneal_ph * KL + \
            sum(reg(w) for w in self.weights_q + self.weights_p)
...
class MultVAERecommender(BaseRecommender, Incremental_Training_Early_Stopping, BaseTempFolder):
    ...
    def _compute_item_score(self, user_id_array, items_to_compute=None):
        ...
        item_scores_to_compute = self.sess.run(self.logits_var, feed_dict={
                                               self.vae.input_ph: URM_train_user_slice})
    ...
    def fit(self,
        ...
        self.vae = _MultVAE_original(
            self.p_dims, q_dims=q_dims, lr=self.learning_rate, lam=self.l2_reg, random_seed=98765)
        ...
        try:
            ...
            self.load_model(self.temp_file_folder,
                            file_name="_best_model", create_zip=False)
    ...
    def _update_best_model(self):
        self.save_model(self.temp_file_folder,
                        file_name="_best_model", create_zip=False)
    ...
    def _run_epoch(self, num_epoch):
        ...
        for bnum, st_idx in enumerate(range(0, self.n_users, self.batch_size)):
            ...
            if self.total_anneal_steps > 0:
                anneal = min(self.anneal_cap, 1. *
                             self.update_count / self.total_anneal_steps)
            else:
    ...
    def save_model(self, folder_path, file_name=None, create_zip=True):
        ...
        dataIO.save_data(file_name="fit_attributes",
...
class MultVAERecommender_OptimizerMask(MultVAERecommender):
    ...
    def fit(self, epochs=100, batch_size=500, total_anneal_steps=200000, learning_rate=1e-3, l2_reg=0.01,
        ...
        p_dims = generate_autoencoder_architecture(
            encoding_size, self.n_items, next_layer_size_multiplier, max_parameters, max_n_hidden_layers)
