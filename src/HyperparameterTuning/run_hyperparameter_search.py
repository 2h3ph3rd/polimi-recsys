...
from src.Recommenders.MatrixFactorization.PureSVDRecommender import PureSVDRecommender, PureSVDItemRecommender
...
from src.Recommenders.MatrixFactorization.Cython.MatrixFactorization_Cython import MatrixFactorization_BPR_Cython,\
...
from src.Recommenders.FactorizationMachines.LightFMRecommender import LightFMItemHybridRecommender, LightFMUserHybridRecommender
from src.Recommenders.FeatureWeighting.Cython.CFW_D_Similarity_Cython import CFW_D_Similarity_Cython
from src.Recommenders.FeatureWeighting.Cython.CFW_DVV_Similarity_Cython import CFW_DVV_Similarity_Cython
from src.Recommenders.FeatureWeighting.Cython.FBSM_Rating_Cython import FBSM_Rating_Cython
    ...
    try:
        ...
        if recommender_class in [ItemKNN_CFCBF_Hybrid_Recommender, UserKNN_CFCBF_Hybrid_Recommender]:
            ...
            if recommender_class is ItemKNN_CFCBF_Hybrid_Recommender:
                ...
                hyperparameters_range_dictionary["UCM_weight"] = Real(
                    low=1e-2, high=1e2, prior='log-uniform')
            ...
            if parallelizeKNN:
                ...
                resultList = pool.map(
                    run_KNNCFRecommender_on_similarity_type_partial, similarity_type_list)
...
def run_KNNRecommender_on_similarity_type(similarity_type, hyperparameterSearch,
    ...
    if similarity_type == "asymmetric":
        ...
        hyperparameters_range_dictionary["normalize_avg_row"] = Categorical([
                                                                            True, False])
        hyperparameters_range_dictionary["similarity_from_distance_mode"] = Categorical([
                                                                                        "lin", "log", "exp"])

    if not is_set_similarity:

        if allow_weighting:
            hyperparameters_range_dictionary["feature_weighting"] = Categorical(
                ["none", "BM25", "TF-IDF"])

        if allow_bias_ICM:
            hyperparameters_range_dictionary["ICM_bias"] = Real(
                low=1e-2, high=1e+3, prior='log-uniform')

        if allow_bias_URM:
            hyperparameters_range_dictionary["URM_bias"] = Real(
                low=1e-2, high=1e+3, prior='log-uniform')

    local_hyperparameter_search_space = {
        **hyperparameters_range_dictionary, **original_hyperparameter_search_space}
...
def runHyperparameterSearch_Collaborative(recommender_class, URM_train, URM_train_last_test=None,
    ...
    try:
        ...
        if recommender_class in [TopPop, GlobalEffects, Random]:
            ...
            hyperparameterSearch = SearchSingleCase(
                recommender_class, evaluator_validation=evaluator_validation, evaluator_test=evaluator_test)
   ...
   ##########################################################################################################
        ...
        if recommender_class is MatrixFactorization_FunkSVD_Cython:
            ...
            if allow_dropout_MF:
                hyperparameters_range_dictionary["dropout_quota"] = Real(
                    low=0.01, high=0.7, prior='uniform')
...
def read_data_split_and_search():
    ...
    from src.Data_manager.DataSplitter_Holdout import DataSplitter_Holdout

    dataset_reader = Movielens1MReader()
    output_folder_path = "result_experiments/SKOPT_test/"

    dataSplitter = DataSplitter_Holdout(
        dataset_reader, user_wise=False, split_interaction_quota_list=[80, 10, 10])
