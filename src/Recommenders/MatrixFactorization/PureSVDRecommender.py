...
def compute_W_sparse_from_item_latent_factors(ITEM_factors, topK=100):
    ...
    similarity_builder = Incremental_Similarity_Builder(
        n_items, initial_data_block=n_items*topK, dtype=np.float32)
    ...
    while start_item < n_items:
        ...
        this_block_weight = np.dot(
            ITEM_factors[start_item:end_item, :], ITEM_factors.T)
        ...
        for col_index_in_block in range(this_block_weight.shape[0]):
            ...
            relevant_items_partition = np.argpartition(
                -this_column_weights, topK-1, axis=0)[0:topK]
            ...
            similarity_builder.add_data_lists(row_list_to_add=relevant_items_partition,
                                              col_list_to_add=np.ones(
                                                  len(relevant_items_partition), dtype=np.int) * item_original_index,
...
class PureSVDItemRecommender(BaseItemSimilarityMatrixRecommender):
    ...
    def __init__(self, URM_train, verbose=True):
        super(PureSVDItemRecommender, self).__init__(
...
class ScaledPureSVDRecommender(PureSVDRecommender):
    ...
    def __init__(self, URM_train, verbose=True):
        super(ScaledPureSVDRecommender, self).__init__(
            URM_train, verbose=verbose)
    ...
    def fit(self, num_factors=100, random_seed=None, scaling_items=1.0, scaling_users=1.0):
        ...
        item_scaling_matrix = sps.diags(
            np.power(item_pop + 1e-6, scaling_items))

        user_pop = np.ediff1d(sps.csr_matrix(self.URM_train).indptr)
        user_scaling_matrix = sps.diags(
            np.power(user_pop + 1e-6, scaling_users))

        self.URM_train = user_scaling_matrix.dot(
            self.URM_train).dot(item_scaling_matrix)

        super(ScaledPureSVDRecommender, self).fit(
            num_factors=num_factors, random_seed=random_seed)
